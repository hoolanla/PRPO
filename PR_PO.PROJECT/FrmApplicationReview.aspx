<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FrmApplicationReview.aspx.cs" Inherits="PR_PO.PROJECT.FrmApplicationReview" %>
        ...
        if (paper_type.Value == "L")
        {
         ...
         for(int i = 0;i < Int32.Parse(page_count.Value );i++)
         {
             ...
             if ((i+1) == Int32.Parse(page_count.Value)) 
             {
              ...
              <div class="row vertical-center-row"  > <img  src="PdfToImageRequest/<%=doc_id.Value%>_<%=i+1%>.PNG" height='<%=h%>' width='<%=w%>' onmousedown='return  getMouseXY(event);'/></div> 
          <%    }
             else
             {
                 %>
                  <div class="row vertical-center-row"  > <img  src="PdfToImageRequest/<%=doc_id.Value %>_<%=i+1%>.PNG" height='<%=h %>' width='<%=w%>'/></div> 
          <%  }
         ...
         { %>
             <div class="row vertical-center-row"><img  src="PdfToImageRequest/<%=doc_id.Value %>.PNG"  height='<%=h %>' width='<%=w%>' onmousedown='return  getMouseXY(event);' id="log" /></div> 
       <%  }
