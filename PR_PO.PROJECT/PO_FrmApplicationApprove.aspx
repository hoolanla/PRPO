<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PO_FrmApplicationApprove.aspx.cs" Inherits="PR_PO.PROJECT.PO_FrmApplicationApprove" %>
...
<div  id="signature1" hidden='hidden'><p id="signature" class="form-pdf"><img src="Images/<%=signature_file.Value %>" width ="<%=ConfigurationSettings.AppSettings["SignWidth"] %>" height="<%=ConfigurationSettings.AppSettings["SignHeight"] %>" /></p></div>
 <div class="container container-table" style="background-color: #ffffff" >
        ...
        if (paper_type.Value == "L")
        {
         ...
         for(int i = 0;i < Int32.Parse(page_count.Value );i++)
         {
             ...
             if ((i+1) == Int32.Parse(page_count.Value)) 
             {
              ...
              <div class="row vertical-center-row"  > <img  src="PO_PdfToImageReview/<%=doc_id.Value%>_<%=i+1%>.PNG" height='<%=h%>' width='<%=w%>' onmousedown='return  getMouseXY(event);'/></div> 
          <%    }
             else
             {
                 %>
                  <div class="row vertical-center-row"  > <img  src="PO_PdfToImageReview/<%=doc_id.Value %>_<%=i+1%>.PNG" height='<%=h %>' width='<%=w%>'/></div> 
          <%  }
         ...
         { %>
             <div class="row vertical-center-row"><img  src="PO_PdfToImageReview/<%=doc_id.Value %>.PNG"  height='<%=h %>' width='<%=w%>' onmousedown='return  getMouseXY(event);' id="log" /></div> 
       <%  }
  ...
  <div class="row vertical-center-row">
      <div class="text-center col-md-1 col-md-offset-1" >
              <asp:Button ID="btnSubmit" runat="server" Text="Go" OnClick="btnSubmit_Click" />    <asp:TextBox ID="txtNot" runat="server"  CssClass="form-control" Width="500" ></asp:TextBox>
 </div>
       </div>
